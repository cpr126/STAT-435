...
```{r chunk5}
# Make sure they are in the same rank, we need to scale it
...
The `center`(mean) and `scale`(sd) components correspond to the means and standard deviations of the variables that were used for scaling prior to implementing PCA.(original dataset)
...
How many principle components should you choose: Look ak the variance that is explained for each
...
TO CHOOSE, we want the sum to be greater than $90\,\%$.
...
```{r chunk29}
km.out <- kmeans(x, 2, nstart = 20) # nstart is how many random starts you want to try
```
...
We begin by clustering observations using complete linkage. The `dist()` function is used to compute the $50 \times 50$ inter-observation Euclidean distance matrix. pair-wise distance
...
hc.single <- hclust(dist(x), method = "single") # worst choise
```
...
```{r chunk37}
cutree(hc.complete, 2) # Perfect
cutree(hc.average, 2) # Not bad, only 3 mistakes
cutree(hc.single, 2) # Worst
```
